...
Deliberately not built: OCR for scanned résumés. If the PDF has no text layer
we detect that and tell the caller, and the UI shows a "type your skills
instead" form. That is a five-minute fallback versus a five-hour rabbit hole,
and roughly one résumé in fifty needs it.
"""
...
def extract_text(pdf_bytes: bytes) -> str:
    ...
    if len(text.strip()) < MIN_CHARS_FOR_TEXT_LAYER:
        raise ScannedResumeError(
            "This PDF has no readable text layer - it looks like a scan or an "
            "image export. Enter your skills manually instead."
        )
    return text
