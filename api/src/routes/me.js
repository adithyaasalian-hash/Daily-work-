...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (file.mimetype !== 'application/pdf') {
      return cb(new Error('Upload a PDF. Export from Word or Docs if you need to.'));
    }
    return cb(null, true);
  },
});
...
router.post(
  ...
  asyncRoute(async (req, res) => {
    if (!req.file) throw httpError(400, 'Attach a PDF résumé.');

    const parsed = await ml.parseResume(req.file.buffer, req.file.originalname);
